...
class ModelArguments:
    ...
    token_mean: Optional[bool] = field(
        default=False,
    )
    ...
    enable_avg_len: Optional[bool] = field(
        default=False,
    )
    topk: Optional[int] = field(
        default=100,
    )
    t2s_only: Optional[bool] = field(
        default=False,
    )
    s2t_only: Optional[bool] = field(
        default=False,
    )
    ...
    class DistillationTrainer(Trainer):
        def __init__(self, *args, teacher_models=None, tokenizers=None, **kwargs):
            ...
            if model_args.teacher_to_student_id_mapping is not None:
                ...
                self.stu2tea_id_mapping_tea = torch.LongTensor(list(self.stu2tea_id_mapping.values())).to(training_args.device)
                self.stu2tea_id_mapping_stu = torch.LongTensor(list(self.stu2tea_id_mapping.keys())).to(training_args.device)
        ...
        def compute_loss(self, model, inputs, return_outputs=False):
            outputs_student = model(input_ids=inputs['input_ids'], attention_mask=inputs['attention_mask'], labels=inputs['labels'], output_hidden_states=True)
            student_loss = outputs_student.loss

            with torch.no_grad():
                outputs_teacher = self.teachers[0](input_ids=inputs['teacher_input_ids0'], attention_mask=inputs['teacher_attention_mask0'], output_hidden_states=True)
             
            kd_loss = 0

            kd_loss = self.compute_uld_loss(torch.clone(outputs_student.logits), torch.clone(outputs_teacher.logits).detach(), inputs)          


            self.distill_metrics.update(sft_loss=student_loss.item(), distill_loss=kd_loss)  
        ...
        def compute_uld_loss(
            ...
            student_target = inputs['labels'].clone()
            teacher_target = inputs['teacher_labels0'].clone()
            # import pdb;pdb.set_trace()
            for i in range(student_target.shape[0]):
                stu_start_idx = student_target[i].ne(-100).nonzero()[0][0]
                stu_start_idx += student_target[i][stu_start_idx:].ne(self.tokenizer.pad_token_id).nonzero()[0][0]  
                tea_start_idx = teacher_target[i].ne(-100).nonzero()[0][0]
                
                tea_start_idx += teacher_target[i][tea_start_idx:].ne(self.teacher_tokenizer.pad_token_id).nonzero()[0][0]
            ...
            tlen = sorted_teacher_probs.shape[1]
            if vocab_size_gap > 0:
                sorted_teacher_probs = torch.cat([
                    sorted_teacher_probs, 
                    torch.zeros(bsz, tlen, vocab_size_gap).float().to(teacher_probs.device)], 
                    dim=-1
                )
            elif vocab_size_gap < 0:
                sorted_student_probs = torch.cat([
                    sorted_student_probs, 
                    torch.zeros(bsz, slen, -vocab_size_gap).float().to(teacher_probs.device)], 
                    dim=-1
                )
            seq_size_gap = student_probs.shape[1] - teacher_probs.shape[1]
            final_vocab_size = sorted_student_probs.shape[-1]
            if seq_size_gap > 0:
                
                sorted_teacher_probs = torch.cat([sorted_teacher_probs,
                                                    (torch.ones(bsz, seq_size_gap, final_vocab_size) * -10000.0).to(teacher_probs.device)],
                                                    dim=-2
                                                )
                teacher_target = torch.cat([teacher_target,
                                            (torch.ones(bsz, seq_size_gap) * -100).to(teacher_probs.device)],
                                            dim=-1)
            elif seq_size_gap < 0:
                sorted_student_probs = torch.cat([sorted_student_probs,
                                                  (torch.ones(bsz, -seq_size_gap, final_vocab_size) * -10000.0).to(student_probs)],
                                                  dim=-2
                                                  )
                student_target = torch.cat([student_target,
                                            (torch.ones(bsz, -seq_size_gap) * -100).to(student_target)],
                                            dim=-1)
            # import pdb;pdb.set_trace()
            ...
            # import pdb;pdb.set_trace()
            uld_loss = (uld_loss * pad_mask).sum() / pad_mask.sum()
            uld_loss = uld_loss.mean()
        ...
        def dist_func(
            ...
            # teacher_temp = model_args.kd_temperature
            logits = logits.float()
            teacher_logits = teacher_logits.float()
            # import pdb;pdb.set_trace()
            lprobs = torch.log_softmax(logits, -1)
            # probs = torch.softmax(logits, -1)
            teacher_probs = torch.softmax(teacher_logits, -1)
            teacher_lprobs = torch.log_softmax(teacher_logits, -1)
            # forward kld
            kld = (teacher_probs * (teacher_lprobs - lprobs))
            # reverse_kld
            # kld = (probs * (lprobs - teacher_probs))
        ...
        def compute_edit_distance_kd_loss(
            ...
            # recover pad id from -100 to the origin padding token
            target_ids = torch.where(
                pad_mask, 
                target, 
                torch.ones_like(target) * self.student_tokenizer.eos_token_id
            )
            ...
            for i in range(bsz):
                ...
                if stu_content_idx.shape[0] == 0 or tea_content_idx.shape[0] == 0:
                    return torch.tensor(0.0).to(training_args.device), 0
                aligned_tea_content_per_step_logits, meaned_stu_content_logits, unmask_rate = self.transform_step_logits_fast(
                    ...
                    tea_input_ids,
                    tea_per_step_logits,
                    blending_to_base_mapping=self.tea2stu_id_mapping,
                    base_to_blending_mapping_blending_ids=self.stu2tea_id_mapping_tea,
                    base_to_blending_mapping_base_ids=self.stu2tea_id_mapping_stu
                )
                # aligned_tea_per_step_logits = student_logits[i].float().detach()
                # change, check and assert, only evaluate response, don't use context.

                # aligned_stu_logits.append(outputs_student.logits[i, stu_content_idx])
        ...
        def transform_step_logits_fast(
            ...
            # tea2stu_mapper = {tea_id: stu_id for stu_id, tea_id in zip(base_to_blending_mapping_base_ids, base_to_blending_mapping_blending_ids)}
            # @timer
            def get_dymaic_mapper(
                    ...
                    blending_topk_tokens, 
                    base_topk_tokens, 
                    # tea2stu_mapper=tea2stu_mapper
                ):
                dist_threashold = 0.3
                converted_student_topk_ids = []
                converted_teacher_topk_ids = []
                for ids, tokens, tea_ids, tea_tokens in zip(base_topk_ids, base_topk_tokens, blending_topk_ids, blending_topk_tokens):
                    # find the tea_topK in the base tokenizer.
                    converted_id_list = []
                    tea_id_list = []
                    for tea_id, tea_tok in zip(tea_ids, tea_tokens):
                        # import pdb;pdb.set_trace()
                        # key judgement
                        if tea2stu_mapper[tea_id]!=0:
                            # import pdb;pdb.set_trace()
                            converted_id_list.append(tea2stu_mapper[tea_id])
                            tea_id_list.append(tea_id)
                        elif model_args.enable_topk:
                            # get the topK tokens that are similar to the base token
                            min_dist = 1000
                            simi_id = 0
                            for id, tok in zip(ids, tokens):
                                tok_dist = dist_fn(tea_tok, tok)
                                if tok_dist<dist_threashold and tok_dist<min_dist:
                                    # print(tok_dist)
                                    simi_id = id
                                    min_dist = tok_dist
                            if simi_id==0:
                                converted_id_list.append(simi_id)
                                tea_id_list.append(simi_id)
                            else:
                                converted_id_list.append(simi_id)
                                tea_id_list.append(tea_id)
                        else:
                            converted_id_list.append(0)
                            tea_id_list.append(0)
                    converted_student_topk_ids.append(converted_id_list)
                    converted_teacher_topk_ids.append(tea_id_list)
                converted_student_topk_ids = torch.LongTensor(converted_student_topk_ids).to(blending_model_per_step_logits.device)
                converted_teacher_topk_ids = torch.LongTensor(converted_teacher_topk_ids).to(blending_model_per_step_logits.device)
                return converted_student_topk_ids, converted_teacher_topk_ids
            
            base_converted_topk_ids, blending_converted_topk_ids = get_dymaic_mapper(blending_topk_ids, base_topk_ids, blending_topk_tokens, base_topk_tokens)
            base_model_per_step_logits = base_model_per_step_logits.gather(-1, base_converted_topk_ids)
            blending_model_per_step_logits = blending_model_per_step_logits.gather(-1, blending_converted_topk_ids)
            base_logit_mask = base_converted_topk_ids.eq(0)            
            base_model_per_step_logits.masked_fill_(base_logit_mask, -100.0)

            blending_logit_mask = blending_converted_topk_ids.eq(0)            
            blending_model_per_step_logits.masked_fill_(blending_logit_mask, -100.0)
            mask_rate = base_logit_mask.sum().item() / (base_logit_mask.size(0) * base_logit_mask.size(1))
            # import pdb;pdb.set_trace()
            return blending_model_per_step_logits, base_model_per_step_logits, 1-mask_rate
