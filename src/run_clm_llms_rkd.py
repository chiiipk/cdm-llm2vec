...
def main():
    ...
    class CustomMetric:
        ...
        def update(self, sft_loss, distill_loss, n=1):
    ...
    class DistillationTrainer(Trainer):
        ...
        def compute_loss(self, model, inputs, return_outputs=False):
            ...
            with torch.no_grad():
                all_teacher_logits = []
                for teacher in self.teachers:
                    outputs_teacher = teacher(input_ids=inputs['input_ids'], attention_mask=inputs['attention_mask'])
                    all_teacher_logits.append(outputs_teacher.logits)
                avg_teacher_logits = all_teacher_logits[0]

     
            if  outputs_student.logits.size() != avg_teacher_logits.size():
                avg_teacher_logits = avg_teacher_logits[:, :, :outputs_student.logits.size(-1)] # batch_size, seq_len, vocab_size
            ...
            loss_logits = (
                loss_function(
                    F.log_softmax(outputs_student.logits / model_args.kd_temperature, dim=-1),
                    F.softmax(avg_teacher_logits / model_args.kd_temperature, dim=-1),
                )* (model_args.kd_temperature ** 2)
            )
            # teacher_probs = F.softmax(avg_teacher_logits / model_args.kd_temperature, dim=-1)
            # student_probs = F.softmax(outputs_student.logits / model_args.kd_temperature, dim=-1)
            # loss_logits = teacher_probs * (teacher_probs.log() - student_probs.log())
            if model_args.token_mean:
                loss_logits = (loss_logits * inputs['attention_mask'].unsqueeze(2)).sum() / inputs['attention_mask'].sum()
            else:
                # loss_logits = loss_logits.sum() / inputs['input_ids'].shape[0]
                loss_logits = (loss_logits * inputs['attention_mask'].unsqueeze(2)).sum() / inputs['attention_mask'].shape[0]
            
            if torch.isnan(loss_logits):
                print(inputs['input_ids'])
                print('NaN Error! debug these examples and delete them')
                exit(-1)
            loss = model_args.kd_alpha * student_loss + (1.0 - model_args.kd_alpha) * loss_logits
            self.distill_metrics.update(sft_loss=student_loss.item(), distill_loss=loss_logits.item())
        ...
        if model_args.freeze_layers is not None or model_args.freeze_emb is not None:
            ...
            for name, param in model.named_parameters():
                ...
                if name == "model.embed_tokens.weight" and model_args.freeze_emb:
                    param.requires_grad = False
                    # freeze_flag = True
                    # print("model.embed_tokens.weight, requires_grad? ", param.requires_grad)
    ...
    # if "llama" in model_args.model_name_or_path or 'cpm' in model_args.model_name_or_path.lower():
    ...
    def preprocess_function(examples):
        ...
        # xxx: 2023-04-07; text: target, prefix: source
            ...
            if "prefix" in column_names:
                ...
                # text = [s + t.replace('</s>', tokenizer.eos_token) for s, t in zip(prefix, text)]
